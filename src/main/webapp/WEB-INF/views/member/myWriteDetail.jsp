	...
	<div class="container my-5 py-5">
		...
		<div class="row mt-4 justify-content-between align-items-center"
			...
			<div class="col-auto" id="board-head-col">
				<i class="fa-regular fa-clock me-3">
					${detail.boardDTO.board_date}</i> <i
					class="fa-regular fa-font-awesome me-3">
					${detail.boardDTO.board_count}</i> <i
					class="fa-regular fa-comment-dots"> ${detail.commentCnt}</i>
			</div>
						...
						<c:forEach items="${detail.commentList}" var="comment">
							<div class="row align-items-center">
								...
								<div class="col-10">
									...
									<div class="row">
										<div class="col-9">
											<textarea class="form-control comment" style="resize: none;"
												readonly>${comment.comment}</textarea>
										</div>
										<c:if test="${loginSession.user_email eq 'admin'}">
											<div class="col-3 defaultComment">
												<button type="button" class="del-commentBtn btn btn-danger"
													value="${comment.seq_comment}">삭제</button>
											</div>
											<div class="col-3 afterComment d-none">
												<button type="button"
													class="btn btn-secondary mod-cancelBtn me-2">취소</button>
												<button type="button"
													class="btn btn-primary mod-completeBtn"
													value="${comment.seq_comment}">완료</button>
											</div>
										</c:if>
										<!-- 댓글 수정/삭제 버튼 -->
										<c:if
											test="${comment.user_email eq loginSession.user_email && loginSession.user_email ne 'admin'}">
											<div class="col-3 defaultComment">
												<button type="button"
													class="mod-commentBtn btn btn-warning me-2"
													value="${comment.seq_comment}">수정</button>
												<button type="button" class="del-commentBtn btn btn-danger"
													value="${comment.seq_comment}">삭제</button>
											</div>
											<div class="col-3 afterComment d-none">
		...
		<form id="commentForm" action="/comment/write" method="post">
			<div class="row mt-4 p-0">
				...
				<div class="col-10">
					...
					<textarea id="inputComment" name="comment" style="resize: none;"
						class="form-control" placeholder="댓글을 입력하세요"></textarea>
				</div>
				<div class="col-2">
					<button type="button" id="write-commentBtn"
						class="btn btn-primary w-100 h-100 fs-5">등록</button>
