						...
						<div class="collapse navbar-collapse justify-content-end"
							...
							<ul class="navbar-nav mb-2 mb-lg-0">
								<li class="nav-item"><a class="nav-link"
									href="/board/toBoard" style="font-size: 18px;">자유게시판</a></li>
		...
		<div id="privacyForm">
			...
			<c:choose>
				<c:when test="${connection.type eq '탈퇴'}">
					<div class="row info">
						<div class="col-md-2"></div>
						<div class="col-md-8">
							<p>${connection.userName}님${connection.userEmail}으로
								${connection.signupDate}에 ${connection.signupType} 회원으로 가입하셨습니다
							</p>
						</div>
						<div class="col-md-2"></div>
					</div>
				...
				<c:when test="${connection.signupType eq '일반'}">
					...
					<div class="row info">
						<div class="col-md-2"></div>
						<div class="col-md-8">
							<p>${connection.signupType}회원에서 ${connection.type} 계정으로 전환을
								원하시면 아래의 버튼을 눌러주세요.</p>
						</div>
						<div class="col-md-2"></div>
					</div>
					<div class="row info">
				...
				<c:when test="${connection.signupType ne '일반'}">
					...
					<div class="row info">
						<div class="col-md-2"></div>
						<div class="col-md-8">
							<p>${connection.signupType}계정으로로그인 해주세요.</p>
						</div>
						<div class="col-md-2"></div>
					</div>
				</c:when>
			...
			<div class="row btnRow">
				<div class="col">
					...
					<c:choose>
						<c:when
							test="${connection.type eq'탈퇴' ||  connection.type eq '블랙리스트'}">
						...
						<c:otherwise>
							<button type="button" id="generalBtn" class="btn">
								<img id="kirikiri" name="kirikiri"
									src="/resources/images/profile.jpg" alt="오류가 발생했습니다.">일반
								회원으로 로그인
							</button>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when
							test="${connection.signupType eq'일반' && connection.type eq '네이버'}">
							<button type="button" id="naverBtn" class="btn">
								<img src="/resources/images/naverLogo.JPG" alt="오류가 발생했습니다.">네이버로
								연동
							</button>
						</c:when>
						<c:when
							test="${connection.signupType eq'일반' && connection.type eq '카카오'}">
							<button type="button" id="kakaoBtn" class="btn">
								<img src="/resources/images/kakoLogo.JPG" alt="오류가 발생했습니다.">카카오로
								연동
							</button>
						</c:when>
					</c:choose>
